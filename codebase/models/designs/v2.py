...
def concat_vec2img(x, s):
    H, W = x._shape_as_list()[1:-1]
    S = s._shape_as_list()[-1]

    # Depth-wise concatenation of s to x
    # Replicate s via broadcasting
    z = tf.ones([1, H, W, 1])
    s = tf.reshape(s, [-1, 1, 1, S])
    s = z * s
    return tf.concat([x, s], axis=-1)
